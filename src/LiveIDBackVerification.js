// src/LiveIDBackVerification.jsx
...
function LiveIDBackVerification() {
  ...
  async function startCamera() {
    ...
    try {
      ...
      ws.onmessage = (evt) => {
        try { setResult(JSON.parse(evt.data)); } catch {}
      };
      wsRef.current = ws;
      startSendingFrames();
    } catch (err) {
  ...
  async function handleCapture() {
    ...
    try {
      ...
      const MIN_RATIO = 0.55; // must match backend MIN_ID_W_RATIO
  ...
  // ---- UI gating (brightness + id detected + inside rect + min fill) ----
  const brightOk = result?.brightness_status === "ok";
  const idOk     = result?.id_card_detected === true;
  const insideOk = result?.id_inside_rect === true;
  const fillOk   = result?.id_fill_ok === true;

  const canCapture = !!(brightOk && idOk && insideOk && fillOk);
  ...
  const guidance = (() => {
    ...
    if (!brightOk) {
      ...
      if (lbl === "too_dark")   return "💡 Image too dark — add light.";
      if (lbl === "too_bright") return "☀️ Image too bright — reduce brightness.";
      return "💡 Adjust lighting.";
    }
    if (!idOk)     return "📇 Place the ID back in view.";
    if (!insideOk) return "📐 Align the ID fully inside the rectangle.";
    if (!fillOk)   return "↔️ Move closer so the ID fills the rectangle.";
  ...
  return (
    <div className="position-relative" style={{ width:"100vw", height:"100dvh", overflow:"hidden", background:"#000" }}>
      ...
      {showMask && (
        <svg width={vp.w} height={vp.h} viewBox={`0 0 ${vp.w} ${vp.h}`}
          ...
          <rect x="0" y="0" width={vp.w} height={vp.h}
                fill="rgba(0,0,0,0.55)" mask="url(#rect-cutout-back)" />
          <rect x={disp.rectX} y={disp.rectY} width={disp.rectW} height={disp.rectH}
        ...
        <div className="d-flex gap-2">
          ...
          {cameraOn && (
            <button className="btn btn-success"
                    ...
                    title={canCapture ? "Capture back of ID" : "Align + lighting to enable capture"}>
      ...
      {isUploading && <BlockingOverlay text="Saving back of ID… Please wait." />}
    </div>
  );
}

export default LiveIDBackVerification;